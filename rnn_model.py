...
class LSTMInferenceModel(object):
    def __init__(self,
        ...
        batch_size = 1
        init_c = [('l%d_init_c_beta'%l, (batch_size, num_hidden)) for l in range(num_lstm_layer)]
        init_h = [('l%d_init_h_beta'%l, (batch_size, num_hidden)) for l in range(num_lstm_layer)]
        ...
        for key in self.executor.arg_dict.keys():
            if key not in input_shapes and key in arg_params and key != "softmax_label":
                arg_params[key].copyto(self.executor.arg_dict[key])

        state_name = []
        for i in range(num_lstm_layer):
            state_name.append("l%d_init_c_beta" % i)
            state_name.append("l%d_init_h_beta" % i)
